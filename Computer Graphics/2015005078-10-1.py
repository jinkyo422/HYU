...
def exp(rv):
    theta = l2norm(rv)
    v = normalized(rv)
    R = np.array([[np.cos(theta) + v[0]*v[0]*(1-np.cos(theta)), v[0]*v[1]*(1-np.cos(theta)) - v[2]*np.sin(theta), v[0]*v[2]*(1-np.cos(theta)) + v[1]*np.sin(theta)],
                  [v[1]*v[0]*(1-np.cos(theta)) + v[2]*np.sin(theta), np.cos(theta) + v[1]*v[1]*(1-np.cos(theta)), v[1]*v[2]*(1-np.cos(theta)) - v[0]*np.sin(theta)],
                  [v[2]*v[0]*(1-np.cos(theta)) - v[1]*np.sin(theta), v[2]*v[1]*(1-np.cos(theta)) + v[0]*np.sin(theta), np.cos(theta) + v[2]*v[2]*(1-np.cos(theta))]])
    return R         

def slerp(R1, R2, t):
    return R1@exp(t*log(R1.T@R2))

def interpolateRotVec(rv1, rv2, t):#lerp(), exp(), log()
    return exp(lerp(rv1, rv2, t))
    
def interpolateZYXEuler(euler1, euler2, t):#lerp(), ZYXEulerToRotMat()
    return ZYXEulerToRotMat(lerp(euler1, euler2, t))

def interpolateRotMat(R1, R2, t):#lerp()
    return lerp(R1, R2, t)
...
def main():
    ...
    if not glfw.init():
        return
    window = glfw.create_window(480,480,'2015005078-10-1', None,None)
    if not window:
        glfw.terminate()
        return
