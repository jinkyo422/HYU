...
run(stateType state, stateType newState)
{
    ...
    while (1) {
        ...
        if (opcode(state.IDEX.instr) == LW) {
            rs = field0(state.IFID.instr);
            rt = field1(state.IFID.instr);
            rd = field1(state.IDEX.instr);
        ...
        if (opcode(state.EXMEM.instr) == ADD) {
            ...
            if (state.EXMEM.aluResult == 0) {// if branch was taken discard registers??
